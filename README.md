...
- 生成覆盖率报告：`cmake -B build -S . -DEVA_ENABLE_COVERAGE=ON && cmake --build build && ctest -L unit && cmake --build build --target coverage`（需要提前安装 `gcovr`，报告输出在 `build/coverage.html` / `build/coverage.xml`）。
