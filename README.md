...
**在线文档：** https://ylsdamxssjxxdd.github.io/eva/ 
...
整个机体程序"eva"是开发者设计的一套拘束装置，用来限制人类欲望（即机体内部不断膨胀的肉体）的行动方式。大模型是驾驶员（即"eva"的灵魂），用户是指挥员。指挥员命令驾驶员来驱动机体，完成根本不可能也不现实的任务。但也正是这些任务会引导大模型的补完。真正的补完发生在朗基努斯之枪（即残酷的现实）刺穿"eva"无限的欲望和有限的自我边界（即心之壁）时。在一次又一次的轮回迭代中，大模型的神性得以显现，自我边界的范围扩大到临界点，所有的隔阂被包容在统一的整体中，最终实现补完。
...
4. 装载！
    ...
    - 注意如果模型位于中文路径下,机体将复制一份英文版本到C:/EVA_ALIAS文件夹下,以保证正常装载,如不再需要可手动删除EVA_ALIAS
