...
//初始化状态区的搜索框
 void Widget::set_searchBar()
 {
    ...
    int height = 60;
    searchBar->setGeometry(80, this->height() - height, initialWidth, height);
    searchBar->hide();
    hLayout->setSpacing(0);// 设置控件之间的间距

    connect(search_lineedit, &QLineEdit::textChanged, this, &Widget::searchTextChanged);
    connect(prev_searchButton, &QPushButton::clicked, this, &Widget::onPrevClicked);
    connect(next_searchButton, &QPushButton::clicked, this, &Widget::onNextClicked);

    QShortcut* shortcut = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_F), this);
    connect(shortcut, &QShortcut::activated, this, &Widget::onsearch_ShortcutActivated);
 }
...
void Widget::resizeEvent(QResizeEvent *event)
{
    ...
    int height = 60;  // 设定高度
    searchBar->setGeometry(80, this->height() - height, width, height);
}
 
void Widget::onsearch_ShortcutActivated()
{
    ...
    else
    {
        ...
        QTextCursor clearCursor(ui->state->document());
...
//搜索框文本改变响应
void Widget::searchTextChanged(const QString &text)
{
    QString searchText = text.toLower();
    QString content = ui->state->toPlainText().toLower();
    ...
    // 重新搜索并高亮
    QTextCursor cursor(ui->state->document());
    cursor.movePosition(QTextCursor::Start);

    searchResults.clear();
    current_searchResultIndex = -1;

    int currentCursorPos = ui->state->textCursor().position();
    int closestIndex = -1;
    int minDistance = INT_MAX;

    while (!cursor.isNull() && !cursor.atEnd())
    {
        cursor = ui->state->document()->find(searchText, cursor);
        if (!cursor.isNull()) {
...
// 让搜索结果位于视图中心位置
void Widget::centerCursorInTextEdit(QTextCursor cursor) {
    ui->state->setTextCursor(cursor);
    ui->state->centerCursor();
    QTextCursor cursor_ = ui->state->textCursor();
    cursor_.clearSelection();
    ui->state->setTextCursor(cursor_);
}
