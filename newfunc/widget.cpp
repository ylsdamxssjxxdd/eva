    ...
    : QWidget(parent), ui(new Ui::Widget)
{
    //---------------初始化ui--------------
    ui->setupUi(this);
    // Ensure tray icon/menu are created before any use
    ...
    connect(ui->output->verticalScrollBar(), &QScrollBar::valueChanged, this, &Widget::output_scrollBarValueChanged);
    ...
    ui_font = QApplication::font(); ui_font.setPointSize(12); // 将设置控件的字体大小设置为10
    EVA_icon = QIcon(":/logo/dark_logo.png");
    QApplication::setWindowIcon(EVA_icon);                                         //设置应用程序图标
    trayIcon->setIcon(EVA_icon);
    trayIcon->setIcon(EVA_icon);
    ui->output->setFont(ui_font);                          // unify output font
    ui->output->document()->setDefaultFont(ui_font);       // enforce default
    ui->state->setFont(ui_font);                           // unify state font
    ui->state->document()->setDefaultFont(ui_font);        // enforce default
    ui->set->setIcon(QIcon(":/logo/assimp_tools_icon.ico"));                       //设置设置图标
    ui->reset->setIcon(QIcon(":/logo/sync.ico"));                                  //设置重置图标
    reflash_state("ui:" + jtr("click load and choose a gguf file"), USUAL_SIGNAL); //初始提示 // 设置系统托盘图标
...
void Widget::recv_pushover()
{
    ...
    if (history_ && ui_state == CHAT_STATE) {
        ...
        if (ui_state == CHAT_STATE && history_) history_->appendMessage(hist);
    }
...
void Widget::on_reset_clicked()
{
    ...
    if (ui_state != COMPLETE_STATE) { resetOutputDocument(); }
    ui_state_normal(); //待机界面状态
