...
8. WPS 专有格式（.et/.dps）  
   - `.et`（WPS 表格）现在直接调用内置的 libxls 解析“Workbook”流，逐 sheet 解析单元格并复用 `makeMarkdownTable` 输出，与 `.xlsx` 一致（`## Sheet N` + Markdown 表格）；读取失败时才回退到旧的 Compound File UTF-16 扫描。  
   - `.dps`（WPS 演示）改为解析 PPT 二进制记录：递归遍历 `PowerPoint Document` 流中的 RecordHeader，识别 TextChars/TextBytes/CString 等文本记录并解码，再按 Markdown 列表输出；若遇到未知结构才回退到早期的 UTF-16 扫描。  
