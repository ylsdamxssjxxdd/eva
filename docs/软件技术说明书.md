...
- 捕捉 prompt done/progress/stop processing 的 n_past → 作为权威矫正值；
...
- UI 输出规则（逐条消息渲染）：
  ...
  - 颜色：`system/user/assistant` 标题为蓝色，`tool` 标题为工具蓝，`think` 标题为灰色；内容：普通为黑色，`think` 内容为灰色。
