...
- xTool 使用 `DockerSandbox`（`src/utils/docker_sandbox.*`）管理容器：  
  ...
  3. 不存在时自动 `docker run -d --name <hash> -w /workspace -v <宿主工作目录>:/workspace <image>`；存在但未运行则 `docker start`；  
  4. 读取容器环境（`uname -sr`、`/etc/os-release`）并通过信号反馈给 UI。
- EVA 会把容器状态附加到工程师提示词中，例如“Docker container (ubuntu:latest)”以及挂载点 `/workspace`。

### 11.2 路径映射与工具执行

- 工程师工作目录默认为 `EVA_WORK`（可自定义），在沙盒模式下以 `/workspace` 挂载。  
...
- 由于所有命令都在 `/workspace` 内执行，请确保工程师工作目录只包含允许模型访问的项目文件。若需隔离多个项目，可为每个项目配置独立的工作目录。
