...
- **第三方组件**：whisper.cpp（ASR）、Stable Diffusion/Flux（文生图）、OuteTTS/WavTokenizer（TTS）。
...
- 若配置缺省，自动从 `EVA_MODELS` 选择最小的 gguf 模型、embedding、whisper、OuteTTS、WavTokenizer、sd1.5-anything-3。
...
| `list_files/search_content/read_file/write_file/edit_file` | 文件读写与检索。 |
...
- **TTS**：OuteTTS/WavTokenizer，增殖窗口按段播放流式输出。
