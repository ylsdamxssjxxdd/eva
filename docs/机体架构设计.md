...
## 1. 架构总览
EVA 是“前端驱动 + 工具协作 + 本地/远端推理兼容”的桌面智能体系统。整体按层拆分：
- **UI 层**：Widget / Expend（交互、状态机、输出渲染、记录条、扩展面板）。
- **Core 层**：会话编排、工具回环、上下文压缩、任务调度（SessionController / ToolFlowController / Scheduler）。
- **Service 层**：网络请求、后端生命周期、工具执行与注册（NetClient / BackendCoordinator / ToolExecutor / ToolRegistry）。
- **Storage 层**：历史记录与向量库（HistoryStore / VectorDB）。
- **外部后端**：llama.cpp server / 远端 OpenAI 兼容 API / MCP / SD / ASR / TTS。

```mermaid
flowchart TB
  UI[UI层\nWidget + Expend]
  CORE[Core层\nSessionController + ToolFlowController + Compaction + Scheduler]
  SERVICE[Service层\nNetClient + BackendCoordinator + ToolExecutor + ToolRegistry]
  STORAGE[Storage层\nHistoryStore + VectorDB]
  BACKEND[外部后端\nllama.cpp / Remote API / MCP / SD / ASR / TTS]
