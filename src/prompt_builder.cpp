        ...
        if (pv.isObject()) {
            ...
            if (type == "text" || type == "image_url" || type == "input_audio") {
                ...
                if (comma != -1) {
                    ...
                    if (header.contains("audio/wav")) format = QStringLiteral("wav");
                    else if (header.contains("audio/mpeg")) format = QStringLiteral("mp3");
                    else if (header.contains("audio/ogg")) format = QStringLiteral("mp3");
                    QJsonObject q; q["type"] = "input_audio";
...
QJsonArray buildOaiChatMessages(const QJsonArray &uiMessages,
    ...
    for (const auto &v : uiMessages) {
        ...
        if (contentVal.isArray()) {
            ...
            if (role == asstRole) {
                QString reasoning, content; splitThink(s, reasoning, content);
    ...
    if (first.value("role").toString() != systemRole) {
        QJsonObject sys; sys.insert("role", systemRole); sys.insert("content", systemPrompt);
        QJsonArray fixed; fixed.append(sys);
