...
inline std::vector<std::string> split(const std::string& s, char delim='+')
{
    ...
    for (char ch : s)
    {
        if (ch==delim) { if (!buf.empty()) out.push_back(buf); buf.clear(); }
...
#ifdef _WIN32
// ---------------- Windows ----------------------------------------------------
    inline LONG toAbs(int v, int full){ return LONG(65535.0 * v / full); }
    ...
    inline void moveCursor(int x,int y)
    {
        ...
        in.mi.dx = toAbs(x,sx);  in.mi.dy = toAbs(y,sy);
        in.mi.dwFlags = MOUSEEVENTF_MOVE|MOUSEEVENTF_ABSOLUTE;
        SendInput(1,&in,sizeof(in));
    }
    inline void mouseFlag(DWORD f)
    {
        INPUT in{}; in.type=INPUT_MOUSE; in.mi.dwFlags=f;
        SendInput(1,&in,sizeof(in));
    }
    inline void leftDown(int x,int y){ moveCursor(x,y); mouseFlag(MOUSEEVENTF_LEFTDOWN); }
    inline void leftUp()             { mouseFlag(MOUSEEVENTF_LEFTUP); }
    inline void rightDown(int x,int y){ moveCursor(x,y); mouseFlag(MOUSEEVENTF_RIGHTDOWN); }
    inline void rightUp()             { mouseFlag(MOUSEEVENTF_RIGHTUP); }
    ...
    inline void sendKeyCombo(const std::vector<std::string>& tokens)
    {
        ...
        for(size_t i = 0; i < tokens.size(); ++i) {
            ...
            else if(t == "WIN")    modifiers.push_back(VK_LWIN);
            else {
                // 非修饰键当作主键（通常是最后一个）
        ...
        // 按下主键
        INPUT in{}; in.type = INPUT_KEYBOARD; in.ki.wVk = mainKey;
        SendInput(1, &in, sizeof(in));
...
// ---------------- Linux X11 ---------------------------------------------------
    ...
    inline void leftDown(int x,int y){ moveCursor(x,y); XTestFakeButtonEvent(dsp(),1,True ,CurrentTime); flush();}
    inline void leftUp()             {                     XTestFakeButtonEvent(dsp(),1,False,CurrentTime); flush();}
    inline void rightDown(int x,int y){ moveCursor(x,y); XTestFakeButtonEvent(dsp(),3,True ,CurrentTime); flush();}
    inline void rightUp()             {                     XTestFakeButtonEvent(dsp(),3,False,CurrentTime); flush();}

    inline KeyCode ks(const std::string& name)
    {
        static std::map<std::string,KeySym> m={
            {"CTRL",XK_Control_L},{"SHIFT",XK_Shift_L},{"ALT",XK_Alt_L},
            {"WIN", XK_Super_L},{"ENTER",XK_Return},{"ESC",XK_Escape},{"TAB",XK_Tab},
